...
function readStoredPopupResult(): PopupMessage | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = window.localStorage.getItem(POPUP_RESULT_KEY);
    if (!raw) return null;
    const data = JSON.parse(raw) as PopupMessage;
    if (!data || data.source !== "grok-auth-popup") return null;
    return data;
  } catch {
    return null;
  }
 ...
 * for the user to dismiss the popup). Also watches localStorage so a preview
 * proxy that breaks `window.opener` can still hand the token back.
 */
function waitForPopupToken(popup: Window): Promise<string | null> {
  return new Promise((resolve) => {
    ...
    const onStorage = (event: StorageEvent) => {
      ...
      try {
        const data = JSON.parse(event.newValue) as PopupMessage;
        if (!data || data.source !== "grok-auth-popup") return;
        settle(data.token ?? null);
      } catch {
        /* ignore malformed */
